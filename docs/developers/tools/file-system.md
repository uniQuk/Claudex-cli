# Claudex file system tools

Claudex provides a comprehensive suite of tools for interacting with the local file system. These tools allow the model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
- **Behavior:**
  ...
  - Respects .gitignore and .claudexignore patterns by default.
...
- **Behavior:**
  ...
  - Respects .gitignore and .claudexignore patterns.
...
- **Behavior:**
  ...
  - **Enhanced Reliability (Multi-Stage Edit Correction):** To significantly improve the success rate of edits, especially when the model-provided `old_string` might not be perfectly precise, the tool incorporates a multi-stage edit correction mechanism.
    - If the initial `old_string` isn't found or matches multiple locations, the tool can leverage the Claudex model to iteratively refine `old_string` (and potentially `new_string`).
...
When reading files, Claudex detects the file's encoding using a multi-step strategy:
...
Set it in `.claudex/settings.json` or `~/.claudex/settings.json`:
...
These file system tools provide a foundation for Claudex to understand and interact with your local project context.

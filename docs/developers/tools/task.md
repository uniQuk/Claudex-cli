...
This document describes the `task` tool for Claudex.
...
You can view available subagents by using the `/agents` command in Claudex.
...
Subagents are configured through Claudex's agent configuration system. Use the `/agents` command to:

...
A channel plugin connects Claudex to a messaging platform. It's packaged as an [extension](../users/extension/introduction) and loaded at startup. For user-facing docs on installing and configuring plugins, see [Plugins](../users/features/channels/plugins).
...
```typescript
import type { ChannelPlugin } from '@claudex/channel-base';
...
```typescript
import { ChannelBase } from '@claudex/channel-base';
import type { Envelope } from '@claudex/channel-base';
...
Your `claudex-extension.json` declares the channel type. The key must match `channelType` in your plugin object:

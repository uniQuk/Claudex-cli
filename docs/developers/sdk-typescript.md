...
A minimum experimental TypeScript SDK for programmatic access to Claudex.
...
```bash
npm install @claudex/sdk
```

## Requirements

- Node.js >= 20.0.0
- [Claudex](https://github.com/ClaudexLM/claudex) >= 0.4.0 (stable) installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the Claudex executable. You should explicitly set the `pathToClaudexExecutable` option to the full path of the `claudex` binary.

## Quick Start

```typescript
import { query } from '@claudex/sdk';
...
Creates a new query session with the Claudex.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'claudex-max'`, `'claudex-plus'`, `'claudex-turbo'`). Takes precedence over `OPENAI_MODEL` and `CLAUDEX_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                 |
| `pathToClaudexExecutable`   | `string`                                       | Auto-detected    | Path to the Claudex executable. Supports multiple formats: `'claudex'` (native binary from PATH), `'/path/to/claudex'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `CLAUDEX_CODE_CLI_PATH` env var, `~/.volta/bin/claudex`, `~/.npm-global/bin/claudex`, `/usr/local/bin/claudex`, `~/.local/bin/claudex`, `~/node_modules/.bin/claudex`, `~/.yarn/bin/claudex`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the Claudex process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in Claudex system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'claudex-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'claudex-oauth'` in SDK is not recommended as credentials are stored in `~/.claudex` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                          |
...
```typescript
const query = claudex.query('Your prompt', {
  timeout: {
...
```typescript
import { query, type SDKUserMessage } from '@claudex/sdk';
...
```typescript
import { query, type CanUseTool } from '@claudex/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@claudex/sdk';
...
```typescript
import { query, isAbortError } from '@claudex/sdk';
...
```typescript
import { AbortError, isAbortError } from '@claudex/sdk';

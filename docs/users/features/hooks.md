...
Claudex hooks provide a powerful mechanism for extending and customizing the behavior of the Claudex application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by Claudex at predefined points in the application flow. They allow users to:
...
Claudex supports three hook executor types:
...
Hooks fire at specific points during a Claudex session. Different events support different matchers to filter trigger conditions.
...
**Purpose**: Executed before Claudex concludes its response to provide final feedback or summaries.
...
{
  "decision": "block",
  "reason": "Must be provided when Claudex is blocked from stopping"
}
```
...
Hooks are configured in Claudex settings, typically in `.claudex/settings.json` or user configuration files:
...
# Log the operation
echo "INFO: Tool $TOOL_NAME executed safely at $(date)" >> /var/log/claudex-security.log
...
Configure in `.claudex/settings.json`:

...
Dual Output is a sidecar mode for the interactive TUI: while Claudex keeps
...
Embed Claudex inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs Claudex with a task prompt. The human sees the TUI in the
...
Integration tests spawn Claudex headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns Claudex
...
```ts
const child = spawn('claudex', ['--json-fd', '3'], {
...
const pty = spawn(
  ...
  [
    '--json-file',
    '/tmp/claudex-events.jsonl',
    '--input-file',
    '/tmp/claudex-input.jsonl',
  ],
  { cols: 120, rows: 40 },
);
```
...
Run Claudex with all three channels enabled:

```bash
mkfifo /tmp/claudex-events.jsonl /tmp/claudex-input.jsonl
claudex \
  --json-file /tmp/claudex-events.jsonl \
  --input-file /tmp/claudex-input.jsonl
```

In a second terminal, tail the event stream:

```bash
cat /tmp/claudex-events.jsonl
```

In a third terminal, push a prompt into the running TUI:

```bash
echo '{"type":"submit","text":"Explain this repo"}' >> /tmp/claudex-input.jsonl
```

The prompt appears in the TUI exactly as if the user typed it, and the
streaming response is mirrored on `/tmp/claudex-events.jsonl`.
...
A typical embedding parent process spawns Claudex with both channels:

```ts
import { spawn } from 'node:child_process';
import { openSync } from 'node:fs';

const eventsFd = openSync('/tmp/claudex-events.jsonl', 'w');
const child = spawn(
  'claudex',
  ['--json-fd', '3', '--input-file', '/tmp/claudex-input.jsonl'],
  { stdio: ['inherit', 'inherit', 'inherit', eventsFd] },
);
```
...
appending JSONL lines to `/tmp/claudex-input.jsonl`.
...
```jsonc
// ~/.claudex/settings.json  (user-level)
// or <workspace>/.claudex/settings.json  (workspace-level)
{
  "dualOutput": {
    "jsonFile": "/tmp/claudex-events.jsonl",
    "inputFile": "/tmp/claudex-input.jsonl",
  },
}
```
...
next Claudex launch, since the bridge is constructed once during
startup.
...
```bash
# Terminal A
mkfifo /tmp/claudex-events.jsonl
cat /tmp/claudex-events.jsonl | jq -c 'select(.type != "stream_event") | {type, subtype}'

# Terminal B
claudex --json-file /tmp/claudex-events.jsonl
...
mkfifo /tmp/claudex-out.jsonl
touch /tmp/claudex-in.jsonl
(cat /tmp/claudex-out.jsonl \
  | jq -c 'select(.type == "control_request")') &

# Terminal B
claudex --json-file /tmp/claudex-out.jsonl --input-file /tmp/claudex-in.jsonl
# Ask Claudex to do something that needs approval, e.g.
...
The most realistic shape: a parent process spawns Claudex, tails
...
const events = join(tmpdir(), `claudex-events-${process.pid}.jsonl`);
const input = join(tmpdir(), `claudex-input-${process.pid}.jsonl`);
writeFileSync(events, '');
writeFileSync(input, '');

const child = spawn('claudex', ['--json-file', events, '--input-file', input], {
  stdio: 'inherit',
});
...
# Claudex TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older Claudex versions won't emit `protocol_version`. Treat the field
...
rl.on('line', (line) => {
  ...
  if (ev.type === 'system' && ev.subtype === 'session_start') {
    ...
    if (v < 1) {
      console.error(
        'claudex dual output is present but protocol < 1; ' +
          'falling back to best-effort behavior',
      );
    } else {
      console.log('claudex dual output protocol v' + v);
    }
  }
});
```

...
This document explains how to run Claudex inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up Claudex:
...
By default, Claudex uses a published sandbox image (configured in the CLI package) and will pull it as needed.

The container sandbox mounts your workspace and your `~/.claudex` directory into the container so auth and settings persist between runs.
...
claudex -s -p "analyze the code structure"

# Or enable sandboxing for your shell session (recommended for CI / scripts)
export CLAUDEX_SANDBOX=true   # true auto-picks a provider (see notes below)
claudex -p "run the test suite"
...
>
> - On **macOS**, `CLAUDEX_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `CLAUDEX_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `CLAUDEX_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `CLAUDEX_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `CLAUDEX_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `CLAUDEX_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/claudexlm/claudex:0.14.1"}}`)

Priority order (highest to lowest):

1. `--sandbox-image`
2. `CLAUDEX_SANDBOX_IMAGE`
3. `tools.sandboxImage`
4. Built-in default image from the CLI package (for example `ghcr.io/claudexlm/claudex:<version>`)

`settings.env.CLAUDEX_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
1. Create a file named `.claudex/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `CLAUDEX_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, Claudex defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.claudex`). Override with:
...
- Container sandbox: add them via `.claudex/sandbox.Dockerfile` or `.claudex/sandbox.bashrc`.
...
The official Claudex Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.claudex/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/claudexlm/claudex:latest
...
```bash
DEBUG=1 claudex -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.claudex/.env` files for Claudex-specific debug settings.

### Inspect sandbox

```bash
# Check environment
claudex -s -p "run shell command: env | grep SANDBOX"

# List mounts
claudex -s -p "run shell command: mount | grep workspace"
```

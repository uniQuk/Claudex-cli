...
Create `.claudex/issues/` if it doesn't exist, then pipe the issue directly
into a markdown file using `gh`:

```bash
mkdir -p .claudex/issues
...
_Pending — to be filled by the test engineer._
' > .claudex/issues/issue-<number>.md
```
...
Spawn the `test-engineer` agent and tell it to read `.claudex/issues/issue-<number>.md`
...
Wait for the test engineer to finish. Then **read `.claudex/issues/issue-<number>.md`**
...
`test-engineer` agent again and tell it to read `.claudex/issues/issue-<number>.md`

...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['CLAUDEX_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    const claudexDir = join(this.testDir, '.claudex');
    ...
    const settings = {
      ...
      sandbox: env.CLAUDEX_SANDBOX !== 'false' ? env.CLAUDEX_SANDBOX : false,
  ...
  /**
   * The command and args to use to invoke Claudex CLI. Allows us to switch
   * between using the bundled gemini.js (the default) and using the installed
   * 'claudex' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const command = isNpmReleaseTest ? 'claudex' : 'node';
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `claudex.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'claudex.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'claudex.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'claudex.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `claudex.${metricName}`) {
              return metric;
            }
          }
        }

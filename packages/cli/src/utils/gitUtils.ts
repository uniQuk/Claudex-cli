...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/claudex/claudex-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest claudex-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest claudex-action release on GitHub.`,
    );
  }
};

...
describe('computeWindowTitle', () => {
  ...
  it('should use default Claudex title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Claudex - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Claudex - project-name_v1.0');
  });
});

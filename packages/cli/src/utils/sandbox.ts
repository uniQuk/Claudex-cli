...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'claudex-sandbox';
const SANDBOX_NETWORK_NAME = 'claudex-sandbox';
const SANDBOX_PROXY_NAME = 'claudex-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under claudex repo
  //
  // note this can only be done with binary linked from claudex repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('claudex/packages/')) {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            CLAUDEX_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the claudex repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify claudex-dev@support if the issue persists.';
    throw new FatalSandboxError(

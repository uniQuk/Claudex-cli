...
export async function parseArguments(): Promise<CliArgs> {
  ...
  if (
    rawArgv.length > 0 &&
    (rawArgv[0].endsWith('/dist/claudex-cli/cli.js') ||
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('claudex')
    .usage(
      'Usage: claudex [options] [command]\n\nClaudex CLI - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('debug', {
      alias: 'd',
      type: 'boolean',
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for Claudex, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch Claudex CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
...
export async function loadCliConfig(
  ...
  // Pass cwd so that relative paths like ".claudex" resolve per-project.
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`claudex --resume\` without an ID to choose from existing sessions.`;
  ...
  const config = new Config({
    sessionId,
    sessionData,
    embeddingModel: DEFAULT_CLAUDEX_EMBEDDING_MODEL,

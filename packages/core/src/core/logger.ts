...
export class Logger {
  ...
  async initialize(): Promise<void> {
    ...
    this.logDir = this.storage.getProjectTempDir();
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.logDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.logDir!, `checkpoint-${tag}.json`);
    try {

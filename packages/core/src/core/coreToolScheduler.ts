...
export class CoreToolScheduler {
      ...
      for (const reqInfo of requestsToProcess) {
        ...
        if (pm && !(await pm.isToolEnabled(reqInfo.name))) {
          ...
          const ruleInfo = matchingRule
            ...
            : '';
          const permissionErrorMessage = `Claudex requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
          newToolCalls.push({
            status: 'error',
            request: reqInfo,
        ...
        if (!pm) {
          ...
          if (excludeTools && excludeTools.length > 0) {
            ...
            if (excludedMatch) {
              const permissionErrorMessage = `Claudex requires permission to use ${excludedMatch}, but that permission was declined.`;
              newToolCalls.push({
                status: 'error',
                request: reqInfo,
                response: createErrorResponse(
        ...
        try {
          ...
          if (approvalMode === ApprovalMode.YOLO && !isAskUserQuestionTool) {
            ...
            if (isNonInteractiveDeny) {
              const errorMessage = `Claudex requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
              this.setStatusInternal(
                reqInfo.callId,
                'error',
                createErrorResponse(
            ...
            if (hooksEnabled && messageBus) {
              fireNotificationHook(
                messageBus,
                `Claudex needs your permission to use ${reqInfo.name}`,
  ...
  private async runConcurrently(
    ...
    const parsed = parseInt(
      process.env['CLAUDEX_CODE_MAX_TOOL_CONCURRENCY'] || '',
      10,
    );

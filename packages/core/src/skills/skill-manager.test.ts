...
describe('SkillManager', () => {
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.claudex/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.claudex/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.claudex/skills/test-skill/SKILL.md';
...
Skill 3 content`);
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.claudex', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.claudex', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.claudex', 'skills');
    const userDirSegment = path.join('.claudex', 'skills');
    ...
    function mockReaddirForLevels(levels: Set<string>) {
      vi.mocked(fs.readdir).mockImplementation((dirPath) => {
        const pathStr = String(dirPath);
        const isBundled =
          pathStr.endsWith(bundledDirSegment) && !pathStr.includes('.claudex');
        const isProject =
...
Symlinked skill content`);
  ...
  describe('file watchers', () => {
    it('should pass ignored function and shallow depth to chokidar', async () => {
      const projectSkillsDir = path.join('/test/project', '.claudex', 'skills');

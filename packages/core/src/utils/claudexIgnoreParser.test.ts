...
import { ClaudexIgnoreParser } from './claudexIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('ClaudexIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'claudexignore-test-'));
  });
  ...
  describe('when .claudexignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .claudexignore', () => {
      const parser = new ClaudexIgnoreParser(projectRoot);
  ...
  describe('when .claudexignore does not exist', () => {

...
`.trim();
  ...
  describe('with claudexignore', () => {
    it('should ignore claudexignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.claudexignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectClaudexIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.claudexignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );

...
describe('quotaErrorDetection', () => {
  describe('isClaudexQuotaExceededError', () => {
    it('should detect the Claudex insufficient_quota error', () => {
      const error = {
        status: 429,
        code: 'insufficient_quota',
        message: 'Free allocated quota exceeded.',
      };
      expect(isClaudexQuotaExceededError(error)).toBe(true);
    });
    ...
    it('should not match when status is not 429', () => {
      ...
      expect(isClaudexQuotaExceededError(error)).toBe(false);
    });
    ...
    it('should not match string errors', () => {
      expect(isClaudexQuotaExceededError('insufficient_quota')).toBe(false);
    });

    it('should not match null or undefined', () => {
      expect(isClaudexQuotaExceededError(null)).toBe(false);
      expect(isClaudexQuotaExceededError(undefined)).toBe(false);
    });
  });
